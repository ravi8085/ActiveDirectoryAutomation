...
$label.Text = 'Enter IPAddresses in below textbox'
...
$HostFQDNTextBox.Size = New-Object System.Drawing.Size(350,225)
...
$OutFile= $PSScriptRoot +"\AddDNSPTRRecordLog.log"
    ...
    $HostFQDNTextBox= $HostFQDNTextBox.Lines
   $LogfileOut = @()
   $IpAddressCount=$IPAddressTextBox.Count
   $HostNamesCount=$HostFQDNTextBox.Count
   Write-Host "$IpAddressCount and $HostNamesCount"
   ...
   $validateIP=[ipaddress]::TryParse($IPAddressTextBox[$i],[ref][ipaddress]::Loopback)
